  ...
  describe "#sort_search_results" do
    ...
    context 'when sort predicate is specified in configuration' do
      ...
      context "and sort term is empty" do
        context "for all" do
          it "does not change order" do
            json_results = [{ label: "['_empty_1_']", sort: [""] }, { label: "['_empty_2_']", sort: [""] }, { label: "['_empty_3_']", sort: [""] }]
            expect(instance.send(:sort_search_results, json_results)).to eq json_results
          end
        end
        ...
        context "for one" do
          it "puts empty first when empty is in 1st position" do
            json_results = [{ label: "['_empty_1_']", sort: [""] }, { label: "[#{term_c}]", sort: [term_c] }, { label: "[#{term_a}]", sort: [term_a] }]
            expected_results = [{ label: "['_empty_1_']" }, { label: "[#{term_a}]" }, { label: "[#{term_c}]" }]
          ...
          it "puts empty first when empty is in 2nd position" do
            json_results = [{ label: "[#{term_b}]", sort: [term_b] }, { label: "['_empty_2_']", sort: [""] }, { label: "[#{term_a}]", sort: [term_a] }]
            expected_results = [{ label: "['_empty_2_']" }, { label: "[#{term_a}]" }, { label: "[#{term_b}]" }]
            expect(instance.send(:sort_search_results, json_results)).to eq expected_results
          end

          it "puts empty first when empty is in last position" do
            json_results = [{ label: "[#{term_b}]", sort: [term_b] }, { label: "[#{term_c}]", sort: [term_c] }, { label: "['_empty_last_']", sort: [""] }]
            expected_results = [{ label: "['_empty_last_']" }, { label: "[#{term_b}]" }, { label: "[#{term_c}]" }]
            expect(instance.send(:sort_search_results, json_results)).to eq expected_results
          end
        end
      end
