...
describe Qa::Authorities::Discogs do
  ...
  describe "#find" do
    context "find variations" do
      ...
      context "json format, no subauthority" do
        let(:tc) { instance_double(Qa::TermsController) }
        let :results do
          authority.find("3380671", tc)
        end
        before do
          allow(Qa::TermsController).to receive(:new).and_return(tc)
          allow(tc).to receive(:params).and_return('format' => "json")
        end
      ...
      context "json-ld format and master subauthority" do
        let(:tc) { instance_double(Qa::TermsController) }
        let :results do
      ...
      context "json-ld format and no subauthority" do
        let(:tc) { instance_double(Qa::TermsController) }
        let :results do
        ...
        before do
          allow(Qa::TermsController).to receive(:new).and_return(tc)
          allow(tc).to receive(:params).and_return('format' => "jsonld")
        end
  ...
  describe "#search" do
    context "search variations" do
      ...
      context "with no subauthority" do
        let(:tc) { instance_double(Qa::TermsController) }
        let :results do
          stub_request(:get, "https://api.discogs.com/database/search?q=melody+gardot+who+will+comfort+me+over+the+rainbo&type=&key=dummy_key&secret=dummy_secret")
        ...
        before do
          allow(Qa::TermsController).to receive(:new).and_return(tc)
          allow(tc).to receive(:params).and_return('subauthority' => "")
        end

        it "has id and label keys" do
          expect(results.first["uri"]).to eq("/Melody-Gardot-Who-Will-Comfort-Me-Over-The-Rainbow/release/1750352")
      ...
      context "with subauthority" do
        ...
        it "has id and label keys" do
          expect(results.first['uri']).to eq "/Wes-Montgomery-Bumpin-On-Sunset-Tequila/master/606116"
