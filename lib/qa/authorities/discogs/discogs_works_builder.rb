  ...
  class Discogs
    module DiscogsWorksBuilder
      ...
      def get_extra_artists_stmts(response)
        ...
        count = 1
        if response["extraartists"].present?
      ...
      def get_styles_stmts(response)
        stmts = []
        if response["styles"].present?
          response["styles"].each do |style|
            # map discogs style to LOC genreForm
            dg = discogs_genres[style.gsub(/\s+/, "")]
            stmts << contruct_stmt_uri_object("Work1", "http://id.loc.gov/ontologies/bibframe/genreForm", dg["uri"]) if dg.present?
            stmts << contruct_stmt_uri_object(dg["uri"], rdf_type_predicate, "http://id.loc.gov/ontologies/bibframe/GenreForm") if dg.present?
            stmts << contruct_stmt_literal_object(dg["uri"], rdfs_label_predicate, dg["label"]) if dg.present?
          end
        end
        stmts # w/out this line, building the graph throws an undefined method `graph_name=' error
      end
      ...
      def get_tracklist_artists_stmts(response)
        ...
        w_count = 2
        if response["tracklist"].present?
          response["tracklist"].each do |track|
            stmts += build_secondary_works(track, w_count)
            ...
            stmts += build_track_artists(artist_array, w_count)
            stmts += build_track_extraartists(track["extraartists"], w_count) if track["extraartists"].present?
