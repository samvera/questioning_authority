...
class Qa::LinkedDataTermsController < ApplicationController
  ...
  def show
    begin
      ...
      title = "Service Unavailable" if e.kind_of? Qa::ServiceUnavailable
      render text: "<h1>#{title}</h1><p>#{e.message}</p>" and return
    end
    render json: term
  end
  ...
  def subauthority
    ...
    def replacement_params
      params.reject { |k,v| ["q", "vocab", "controller", "action"].include?(k) }
    end
end
